...
public class WechatAuthorizationRequestConsumer implements Consumer<OAuth2AuthorizationRequest.Builder> {
    ...
    public void accept(OAuth2AuthorizationRequest.Builder builder) {
        ...
        if (Objects.equals(registrationId, DefaultConstants.THIRD_LOGIN_WECHAT)) {
            // 判断是否微信登录，如果是微信登录则将appid添加至请求参数中
            builder.additionalParameters((params) -> params.put(DefaultConstants.WECHAT_PARAMETER_FORCE_POPUP, true));
            builder.additionalParameters((params) -> params.put(DefaultConstants.WECHAT_PARAMETER_APPID, authorizationRequest.getClientId()));
        }
    }
